...
- 번역 결과를 TXT, HTML 또는 EPUB으로 저장해 오프라인으로 보관할 수 있습니다.
...
6. 읽은 결과를 TXT, HTML 또는 EPUB으로 내보냅니다.

> [!TIP]
> 처음에는 짧은 챕터 하나로 모델, 프롬프트, 치환 규칙이 마음에 드는지 확인해 두면 이후 읽는 흐름이 훨씬 편합니다.

## 고급 설정: 외부 YAML로 LLM 고정하기

고급 사용자라면 앱 UI 대신 외부 `config.yaml` 파일로 LLM 제공자, 모델, 스트리밍 여부를 고정할 수 있습니다.

- macOS: `~/.config/transnovel/config.yaml`
- Windows: `%APPDATA%\transnovel\config.yaml`
- Linux: `~/.config/transnovel/config.yaml`

시작할 때는 저장소의 [config.example.yaml](./config.example.yaml)을 복사해 값을 바꾸는 편이 가장 빠릅니다.
...
- 파일이 존재하면 앱의 `LLM 설정` UI는 읽기 전용으로 잠깁니다.
- YAML 방식은 API 키 기반 제공자만 지원합니다. `OpenAI (Codex)` OAuth 로그인은 이 모드에서 사용할 수 없습니다.
- 파일 형식이 잘못되면 앱이 설정 오류를 표시하고 조용히 DB 설정으로 fallback 하지 않습니다.
