...
| 2026-04-17 | GitHub Issues를 backlog와 non-trivial task context의 source of truth로 사용하고, `.context/TASKS.md`는 로컬 실행 스냅샷으로 유지하기로 함 (2026-07-27 결정으로 폐기) | 세션 간 연속성을 GitHub에 남기고, 로컬 tracker와 장기 backlog의 역할을 분리하기 위한 당시 결정 |
| 2026-04-17 | 작업 문서가 필요하면 `docs/` 아래에 두고, 복잡한 작업에만 작성한다 | `mydocs/`식 문서 과잉을 피하면서도 재사용 가능한 설계/조사 결과는 저장하기 위해 |
| 2026-07-27 | GitHub Issues를 task, backlog, plan, progress, blocker, verification, completion state의 유일한 source of truth로 사용하고 로컬 `.context/TASKS.md` tracker를 폐기한다 | 하나의 작업 상태를 여러 tracker에 중복 기록해 생기는 불일치를 제거하고, 미완료 작업은 각각 독립 Issue로 관리하기 위해 |
